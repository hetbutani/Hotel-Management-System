...
# Fix the scope of renderRooms and the fetch URLs
# We'll pull renderRooms out of the try block and make it available to the search button
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Initial load
    try {
        const response = await fetch('http://127.0.0.1:5001/api/rooms/featured');
    ...
    // Search Logic
    const searchBtn = document.getElementById('check-availability');
    searchBtn.addEventListener('click', async () => {
        ...
        container.innerHTML = '<p class="text-center col-span-full font-body-md text-on-surface-variant">Searching for available rooms...</p>';

        try {
            const response = await fetch('http://127.0.0.1:5001/api/rooms/search', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ check_in: checkIn, check_out: checkOut, guests: guests })
            });
            const rooms = await response.json();
            renderRooms(rooms);
            ...
            container.innerHTML = '<p class="text-center col-span-full font-body-md text-error">Failed to search rooms. Please try again later.</p>';
        }
    });
});
</script>
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Successfully fixed scope issues in code.html.")
